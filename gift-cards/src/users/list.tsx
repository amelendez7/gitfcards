...
const AddUserModal: FC = function () {
  ...
  const handleSubmit = async (e: React.FormEvent, id_rol: any, id_job: any) => {
    e.preventDefault()
    try {
      const response = await axios.post('https://bn.glassmountainbpo.com:8080/api/save', {
        badge,
        username,
        password,
        id_rol,
...
const EditUserModal: FC<any> = function ({ badge, username, userDepartment, userPosition, status, created_user, id_rol, sharedState, updateSharedState }: any) {
  const [isOpen, setOpen] = useState(false);
  const [password, setPassword] = useState<any>('');
...
const DeleteUserModal: FC<any> = function ({ badge, status, created_user, sharedState, updateSharedState }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent, badge: any, status: any, created_user: any) => {
    e.preventDefault()
    try {
      const response = await axios.post('https://bn.glassmountainbpo.com:8080/api/status', {
        badge,
        status,
        created_user
      })
